"""
SDPose-S + Entropy-Weighted KD (CrowdPose, v3.5 기반)
=====================================================

COCO v3.5 설정을 CrowdPose에 이식:
- v3.5의 모든 최적화 유지 (EW Loss Weight 증가)
- loss_ew_heatmap: 8.0e-6 → 1.0e-5
- loss_ew_token_vis: 8.5e-6 → 1.0e-5
- loss_ew_token_kpt: 8.5e-6 → 1.0e-5

예상 효과: COCO와 유사하게 +0.3~0.5%p AP 향상
목표: v3 (64.6%) → v3.5 (65.0~65.2% AP)
"""

# CrowdPose dataset 정의는 mmpose 쪽 베이스 설정을 재사용
_base_ = ['../../../../../mmpose/configs/_base_/datasets/crowdpose.py']

date = '1221'
exp_description = 'crowdpose_entropy_weighted_tuned_v3.5'
...
# ============ Optimizer (v3.5와 동일, no clipping) ============
optimizer = dict(
    type='Adam',
    lr=1e-3,
)
optimizer_config = dict(grad_clip=None)  # v3.5와 동일

# ============ Learning Rate (v3.5와 동일) ============
lr_config = dict(
    policy='step',
    warmup='linear',
    warmup_iters=900,
...
# ============ Model (Entropy-Weighted KD v3.5, SDPose-S) ============
model = dict(
    type='TopDown',
    ...
    keypoint_head=dict(
        ...
        # Baseline token distillation (v3.5와 동일)
...
data_cfg = dict(
    ...
    oks_thr=0.9,
    vis_thr=0.2,
    use_gt_bbox=True,  # Use GT bbox if detection file not available
    det_bbox_thr=0.0,
    bbox_file='',  # Empty string to use GT bbox, or path to detection file
)
