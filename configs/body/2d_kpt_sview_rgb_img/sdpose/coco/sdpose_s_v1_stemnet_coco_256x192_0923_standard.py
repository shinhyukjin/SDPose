...
model = dict(
    ...
    keypoint_head=dict(
        ...
        # 2. Visual Token은 기존의 값 기반 증류를 유지합니다.
        loss_vis_token_dist=dict(type='TokenDistilLoss', loss_weight=5e-6),

        # 3. Keypoint Token에만 '관계 기반 증류'를 적용합니다. (이 부분은 유지)
        loss_kpt_token_dist=dict(type='RelationalTokenDistilLoss', loss_weight=0.1, loss_type='L1'),

        # 4. 성공했던 어텐션 증류는 그대로 유지합니다. (이 부분은 유지)
